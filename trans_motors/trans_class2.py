...
class TransMotor():
	...
	def __init__(self, name, address, sign):
		...
		rospy.Subscriber('/encoder_positions', Twist, self.encoder_position)
		rospy.Subscriber('/target_angle', Twist, self.callback)
		rospy.Subscriber('/reconfig', Twist, self.reconfig)
		rospy.Subscriber('/can_encoder', Twist, self.robot_width) # CHANGE THE TOPIC NAME
		...
		#self.motor_rpm = 0
		self.position = 0 # steering angle
		self.complement = 0
	...
	def reconfig(self, data): ###
		...
		self.wheel_speed = self.rads_to_rpm(self.reconfig_speed / 0.1)
	...
	def motor_lin_vel(self, vx, wz):
		sign = wz / abs(wz)
		r = abs(vx / wz)
		speed = (math.sqrt((r - sign*self.width/2)**2 + (self.length/2)**2) * abs(wz)) # check + or - 
		if r < self.width/2 and vx/wz < 0:
			speed  = self.sign*speed
		elif r < self.width/2 and vx/wz > 0:
			speed = -self.sign*speed
		#print(r)
